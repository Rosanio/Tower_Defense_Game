...
gEngine.Textures = (function() {
    //Loads a texture so it can be drawn
    ...
    var loadTexture = function(textureName) {
        if(!(gEngine.ResourceMap.isAssetLoaded(textureName))) {
            ...
            //When the texture loads, convert it to the WebGL format then put it back into the mTextureMap
            img.onload = function() {
    ...
    //Remove the reference to allow associated memory to be available for subsequent garbage collection
    ...
    var activateTexture = function(textureName) {
        ...
        //For pixel graphics where you want the texture to loop "sharp"
        //    do the following
    ...
    var getColorArray = function(textureName) {
        ...
        if(texInfo.mColorArray === null) {
            //create a frame buffer and bind it to the texture, and read the color content
            var gl = gEngine.Core.getGL();
            ...
            if(gl.checkFramebufferStatus(gl.FRAMEBUFFER) === gl.FRAMEBUFFER_COMPLETE) {
                ...
                alert("Warning: Engine.Texture.GetColorArray() failed!");
            }
